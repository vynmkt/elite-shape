// api/[...slug].ts — Catch-all handler for all /api/* routes except auth and ai
import type { VercelResponse } from '@vercel/node';
import { withAuth, withAdminAuth, AuthRequest } from './_auth';
...
export default withAuth(async (req: AuthRequest, res: VercelResponse) => {
  ...
  if (path === '/profile') {
    if (method === 'GET') {
      ...
      const user = await queryOne('SELECT name, is_premium, role, points, theme, language FROM users WHERE id = $1', [userId]);
  ...
  // ── NUTRITION ────────────────────────────────────────────────
  if (path === '/nutrition/daily' && method === 'GET') {
  ...
  // ── WEIGHT ───────────────────────────────────────────────────
  if (path === '/weight/history' && method === 'GET') {
    return res.json(await query('SELECT * FROM weight_history WHERE user_id = $1 ORDER BY timestamp ASC', [userId]));
  }
  ...
  if (path === '/water' && method === 'POST') {
    await queryRun('INSERT INTO water_logs (user_id,amount) VALUES ($1,$2)', [userId, req.body.amount]);
    return res.json({ success: true });
  }
  ...
  // ── SHAPE ────────────────────────────────────────────────────
  if (path === '/shape/history' && method === 'GET') {
    return res.json(await query('SELECT * FROM shape_history WHERE user_id = $1 ORDER BY timestamp DESC', [userId]));
  }
  ...
  if (path === '/leaderboard' && method === 'GET') {
    return res.json(await query(`SELECT users.name, users.points, COALESCE(profiles.level,'Frango') as level, COALESCE(profiles.streak,0) as streak FROM users LEFT JOIN profiles ON users.id=profiles.user_id ORDER BY users.points DESC LIMIT 10`));
  }

  // ── TRAINING ─────────────────────────────────────────────────
  if (path === '/training/loads' && method === 'GET') {
    return res.json(await query('SELECT * FROM load_tracking WHERE user_id=$1 ORDER BY timestamp DESC', [userId]));
  }
  ...
  if (path === '/usage/log' && method === 'POST') {
    await queryRun('INSERT INTO usage_logs (user_id,type,tokens) VALUES ($1,$2,$3)', [userId, req.body.type, req.body.tokens || 0]);
    return res.json({ success: true });
  }
  if (path === '/logs/error' && method === 'POST') {
    await queryRun('INSERT INTO error_logs (user_id,error_message,stack_trace,context) VALUES ($1,$2,$3,$4)', [userId, req.body.message, req.body.stack, JSON.stringify(req.body.context)]);
    return res.json({ success: true });
  }

  // ── USER SETTINGS ────────────────────────────────────────────
  if (path === '/user/settings' && method === 'POST') {
    await queryRun('UPDATE users SET theme=$1, language=$2 WHERE id=$3', [req.body.theme || 'dark', req.body.language || 'pt', userId]);
    return res.json({ success: true });
  }
  ...
  if (path.startsWith('/admin')) {
    // Check admin
    const adminUser = await queryOne('SELECT role FROM users WHERE id = $1', [userId]);
    if (!adminUser || adminUser.role !== 'admin') return res.status(403).json({ error: 'Acesso negado' });

    if (path === '/admin/users' && method === 'GET') {
      const search = `%${req.query.search || ''}%`;
      return res.json(await query('SELECT id,name,email,is_premium,role,created_at FROM users WHERE name ILIKE $1 OR email ILIKE $2 ORDER BY created_at DESC', [search, search]));
    }
    if (path === '/admin/stats' && method === 'GET') {
      const total = await queryOne('SELECT COUNT(*) as count FROM users');
      const premium = await queryOne('SELECT COUNT(*) as count FROM users WHERE is_premium = 1');
      const tokens = await queryOne('SELECT SUM(tokens) as count FROM usage_logs');
    ...
    if (path === '/admin/ai-settings' && method === 'GET') {
      return res.json({ active_ai_provider: 'openai', openai_api_key: '****', ai_fallback_enabled: '0' });
    }
    if (path === '/admin/ai-settings' && method === 'POST') {
      return res.json({ success: true });
    }
    const idMatch = path.match(/^\/admin\/user\/(\d+)\/plan$/);
    if (idMatch && method === 'POST') {
      await queryRun('UPDATE users SET is_premium = $1 WHERE id = $2', [req.body.is_premium ? 1 : 0, idMatch[1]]);
      return res.json({ success: true });
    }
    const statsMatch = path.match(/^\/admin\/user\/(\d+)\/stats$/);
    if (statsMatch && method === 'GET') {
      return res.json(await query('SELECT type, SUM(tokens) as total_tokens, COUNT(*) as count FROM usage_logs WHERE user_id=$1 GROUP BY type', [statsMatch[1]]));
    }
  }
